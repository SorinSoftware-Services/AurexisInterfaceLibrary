...
return function(Window, Aurexis, Elements, Navigation, GetIcon, Kwargify, tween, Release, isStudio)
	...
	local function ensureDetailsScroller(detailsHolderRef, dashboardRef)
		...
		if not padding then
			...
			padding.PaddingBottom = UDim.new(0, 200)
			padding.Parent = container
		else
			padding.PaddingRight = UDim.new(0, math.max(padding.PaddingRight.Offset, 10))
			padding.PaddingBottom = UDim.new(0, math.max(padding.PaddingBottom.Offset, 200))
		end
		...
		if container:IsA("ScrollingFrame") then
			local okAuto = pcall(function()
				return container.AutomaticCanvasSize == Enum.AutomaticSize.Y
			end)
			if not okAuto then
