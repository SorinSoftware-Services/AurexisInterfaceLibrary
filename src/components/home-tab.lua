...
return function(Window, Aurexis, Elements, Navigation, GetIcon, Kwargify, tween, Release, isStudio)
	...
	local function createContentFrame(card, name, allowScroll)
		local content = card:FindFirstChild(name)
		if content then
	...
	local function createInputField(parent, font, placeholder, maxChars)
	...
	local function buildFeedbackCard(card)
		...
		local feedbackBox, feedbackFrame = createInputField(content, font, "What should we improve?", 300)
		feedbackFrame.LayoutOrder = 2

		local ideaBox, ideaFrame = createInputField(content, font, "Game ideas or feature requests", 200)
		ideaFrame.LayoutOrder = 3

		local contactBox, contactFrame = createInputField(content, font, "Contact (optional)", 80)
		...
		submitButton.BackgroundColor3 = Color3.fromRGB(90, 120, 200)
	...
	local function buildTeleportCard(card)
		...
		safeTitle.Size = UDim2.new(1, 0, 0, 16)
		safeTitle.LayoutOrder = 1
		...
		safeDesc.Size = UDim2.new(1, 0, 0, 30)
		safeDesc.LayoutOrder = 2
		safeDesc.Parent = content
		...
		teleportButton.MouseButton1Click:Connect(function()
			...
			if not entry or not entry.placeId then
	...
	local function loadGameEntries()
		if type(HomeTabSettings.GameEntries) == "table" then
			...
			for _, entry in ipairs(HomeTabSettings.GameEntries) do
				if type(entry) == "table" and entry.name and entry.placeId then
					table.insert(entries, {
						name = entry.name,
						placeId = entry.placeId,
						universeId = entry.universeId,
		...
		return buildEntriesFromManager(manager)
	end
